     ...
     if not uFuncoes.Empty(edtNRMESA.Text) Then
       begin
            If (StrToInt(edtNRMESA.Text) > dm.CdsConfigCFG_QTMESAS.AsInteger) Then
             begin
                   Application.MessageBox (PChar('Número de mesa inválido !'),
   ...
   if (dmDados.cdsItensVenda.State in [dsInsert, dsEdit]) Then
    begin
      ...
      Begin
           ...
           Begin
                Application.MessageBox (PChar('Código de atendente não cadastrado !'),
  ...
  inherited;
   if (dmDados.cdsItensVenda.State in [dsInsert, dsEdit]) Then
    begin
      If uFuncoes.Empty(dbeProduto.Text) Then
        begin
             Application.MessageBox (PChar('Digite o código de produto.'),
                  'ATENÇÃO',MB_ICONINFORMATION+ MB_OK);
              dbeProduto.Clear;
              dbeProduto.SetFocus;
              Exit;
          ...
          if (dmConsultas.GetProduto(dmDados.cdsItensVendaPVI_PRODUTO.AsString)) Then
           begin
               ...
               if not (Dm.AddServico(dmDados.cdsPreVendaPRV_NRMESA.AsString,
                   ...
                   Application.MessageBox (PChar('Erro na inclusão de serviço !'),
 ...
 // inherited;
     if (dmDados.cdsItensVenda.Active) and not (dmDados.cdsItensVenda.IsEmpty) Then
        if (dmDados.cdsItensVenda.State in [dsbrowse]) Then
           If Application.MessageBox('Deseja excluir item selecionado ?', 'Exclusão de registro',
             ...
             Begin
                  ...
                  If (M_NRSEQU > 0) Then
                     if not (Dm.ExcluirServico(M_NRSEQU)) Then
                        ShowMessage('Erro ao tentar excluir serviço de produto !');
                  //
          ...
          If (key = vk_F2) and (dmDados.cdsPreVenda.Active)
           ...
           Begin
                ...
                {If not (dmDados.cdsItensVenda.Active) or (dmDados.cdsItensVenda.IsEmpty) Then
                    begin
                         Application.MessageBox (PChar('Não há itens no pedido atual !'+#13+'Inclua itens para poder fechar pedido.'),
                            'ATENÇÃO',MB_ICONINFORMATION+ MB_OK);
                         Exit;
                    End;
                 //
                      ...
                      Begin
                           ...
                           If (Dm.VerificarServicoMesa(W_NRMESA)) Then
                              if not (dm.AtualizaStatusServicoMesa(W_NRMESA, 'E')) then
                                   Application.MessageBox (PChar('Erro ao tentar atualizar status de serviço !'),
                                          'ATENÇÃO',MB_ICONSTOP+ MB_OK);
           ...
           Begin
                ...
                If not (dmDados.cdsItensVenda.Active) or (dmDados.cdsItensVenda.IsEmpty) Then
                 ...
                 If (Copy(W_NRMESA,1,1) = 'T') Then
                   begin
                        aValor := InputBox('Taxa de Serviço','Digite o valor da taxa de serviço:','');
                        ...
                        begin
                          try
                              ...
                              W_FLTAXA := True;
                          Except
                             Application.MessageBox (PChar('Valor da taxa inválido.'),
                                'ATENÇÃO',MB_ICONINFORMATION+ MB_OK);
                             Exit;
                          End;
                        End
                   ...
                   End
                 Else
                     If (Application.MessageBox('Deseja incluir taxa de serviço ?',
...
begin
  inherited;
   if (dmDados.cdsPreVenda.Active) and not (dmDados.cdsPreVenda.IsEmpty) Then
         If Application.MessageBox('Confirma Exclusão de Pedido ?',
             'ATENÇÃO', MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2+MB_APPLMODAL) = idYes then
             begin
                  if (Dm.ExcluirPedido(dmDados.cdsPreVendaPRV_PEDIDO.AsInteger)) Then
                   begin
                        Application.MessageBox(PChar('Pedido excluído com sucesso.'),
                          'ATENÇÃO', MB_OK+MB_ICONINFORMATION+MB_APPLMODAL);
                        //
    ...
    begin
         ...
         If (Copy(W_NRMESA,1,1) = 'T') Then
           ...
           Else
            begin
                //
                If Application.MessageBox('Incluir taxa de serviço ?',
                 'ATENÇÃO', MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON1+MB_APPLMODAL) = idYes then
