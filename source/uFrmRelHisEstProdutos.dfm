object frmRelHistEstProdutos: TfrmRelHistEstProdutos
  Left = 285
  Top = 222
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Relatorio de Historico Estoque de Produtos'
  ClientHeight = 214
  ClientWidth = 364
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlDados: TPanel
    Left = 0
    Top = 0
    Width = 364
    Height = 173
    Align = alClient
    TabOrder = 0
    object Label1: TLabel
      Left = 16
      Top = 21
      Width = 38
      Height = 13
      Caption = '&Per'#237'odo'
      FocusControl = edtDTINIC
      Transparent = True
    end
    object Label2: TLabel
      Left = 16
      Top = 61
      Width = 40
      Height = 13
      Caption = '&Produto:'
      FocusControl = edtDTINIC
      Transparent = True
    end
    object spProdutos: TSpeedButton
      Left = 138
      Top = 80
      Width = 23
      Height = 22
      Hint = 'Localizar Produto'
      Flat = True
      Glyph.Data = {
        
        777777777770B777777777777777777777777777777777777777}
      OnClick = spProdutosClick
    end
    object lbl_NMPROD: TLabel
      Left = 18
      Top = 107
      Width = 13
      Height = 13
      Caption = '...'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object edtDTINIC: TDateEdit
      Left = 16
      Top = 36
      Width = 121
      Height = 21
      NumGlyphs = 2
      TabOrder = 0
      OnExit = edtDTINICExit
    end
    object edtDTFINA: TDateEdit
      Left = 152
      Top = 36
      Width = 121
      Height = 21
      NumGlyphs = 2
      TabOrder = 1
      OnExit = edtDTFINAExit
    end
    object edtCDPROD: TEdit
      Left = 16
      Top = 80
      Width = 121
      Height = 21
      MaxLength = 13
      TabOrder = 2
      OnChange = edtCDPRODChange
      OnExit = edtCDPRODExit
      OnKeyPress = edtCDPRODKeyPress
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 173
    Width = 364
    Height = 41
    Align = alBottom
    Color = clNavy
    TabOrder = 1
    object BtSair: TBitBtn
      Left = 275
      Top = 10
      Width = 75
      Height = 25
      Caption = '&Fechar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BtSairClick
      Glyph.Data = {
        
        1B1C010303030303031A1A1A1A1A1A1A0303011A1A1A1A1A1A1A}
    end
    object btImprimir: TBitBtn
      Left = 187
      Top = 9
      Width = 75
      Height = 25
      Caption = '&Imprimir'
      TabOrder = 0
      OnClick = btImprimirClick
      Glyph.Data = {
        
        0000000000000000DBDBDBDBDBDBDBDBDBDBDBDBDBDBDBDBDBDB}
    end
  end
  object dstHistorico: TSQLDataSet
    CommandText = 
      'Select H.HIS_PRODUTO, H.HIS_DATA, H.his_tipo, H.his_documento,'#13#10 +
      'H.his_anterior, H.HIS_MOVIMENTO, H.his_atual,'#13#10'(Select U.SEN_NOM' +
      'E from USUARIOS U Where (U.SEN_CODIGO = H.HIS_USUARIO)) As NOMEU' +
      'SUARIO'#13#10'from HISTORICO H'#13#10'Where (H.HIS_PRODUTO = :pPRODUTO) and'#13 +
      #10'(H.his_data >= :pDTINICIAL) AND (H.his_data <= :pDTFINAL)'#13#10
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'pPRODUTO'
        ParamType = ptInput
        Value = ''
      end
      item
        DataType = ftDate
        Name = 'pDTINICIAL'
        ParamType = ptInput
        Value = 0d
      end
      item
        DataType = ftDate
        Name = 'pDTFINAL'
        ParamType = ptInput
        Value = 0d
      end>
    SQLConnection = Dm.SqlAdmin
    Left = 8
    Top = 181
  end
  object dspHistorico: TDataSetProvider
    DataSet = dstHistorico
    Options = [poAllowCommandText]
    Left = 48
    Top = 181
  end
  object cdsHistorico: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    ProviderName = 'dspHistorico'
    OnCalcFields = cdsHistoricoCalcFields
    Left = 88
    Top = 181
    object cdsHistoricoHIS_PRODUTO: TStringField
      FieldName = 'HIS_PRODUTO'
      FixedChar = True
      Size = 13
    end
    object cdsHistoricoHIS_DATA: TDateField
      FieldName = 'HIS_DATA'
    end
    object cdsHistoricoHIS_TIPO: TStringField
      FieldName = 'HIS_TIPO'
      FixedChar = True
      Size = 2
    end
    object cdsHistoricoHIS_DOCUMENTO: TIntegerField
      FieldName = 'HIS_DOCUMENTO'
    end
    object cdsHistoricoHIS_ANTERIOR: TFMTBCDField
      FieldName = 'HIS_ANTERIOR'
      Precision = 15
      Size = 3
    end
    object cdsHistoricoHIS_ATUAL: TFMTBCDField
      FieldName = 'HIS_ATUAL'
      Precision = 15
      Size = 3
    end
    object cdsHistoricoNOMEUSUARIO: TStringField
      FieldName = 'NOMEUSUARIO'
      Size = 40
    end
    object cdsHistoricoHIS_NOMETIPO: TStringField
      FieldKind = fkCalculated
      FieldName = 'HIS_NOMETIPO'
      Size = 15
      Calculated = True
    end
    object cdsHistoricoHIS_MOVIMENTO: TFMTBCDField
      FieldName = 'HIS_MOVIMENTO'
      Precision = 15
      Size = 3
    end
    object cdsHistoricoHIS_NRDOCU: TStringField
      FieldKind = fkCalculated
      FieldName = 'HIS_NRDOCU'
      Size = 7
      Calculated = True
    end
  end
  object dsHistorico: TDataSource
    DataSet = cdsHistorico
    Left = 120
    Top = 181
  end
  object ppDBHistorico: TppDBPipeline
    DataSource = dsHistorico
    UserName = 'DBHistorico'
    Left = 312
    Top = 80
    object ppDBHistoricoppField1: TppField
      FieldAlias = 'HIS_PRODUTO'
      FieldName = 'HIS_PRODUTO'
      FieldLength = 0
      DisplayWidth = 0
      Position = 0
    end
    object ppDBHistoricoppField2: TppField
      FieldAlias = 'HIS_DATA'
      FieldName = 'HIS_DATA'
      FieldLength = 0
      DataType = dtDate
      DisplayWidth = 10
      Position = 1
    end
    object ppDBHistoricoppField3: TppField
      FieldAlias = 'HIS_TIPO'
      FieldName = 'HIS_TIPO'
      FieldLength = 2
      DisplayWidth = 2
      Position = 2
    end
    object ppDBHistoricoppField4: TppField
      Alignment = taRightJustify
      FieldAlias = 'HIS_DOCUMENTO'
      FieldName = 'HIS_DOCUMENTO'
      FieldLength = 0
      DataType = dtInteger
      DisplayWidth = 10
      Position = 3
    end
    object ppDBHistoricoppField5: TppField
      Alignment = taRightJustify
      FieldAlias = 'HIS_ANTERIOR'
      FieldName = 'HIS_ANTERIOR'
      FieldLength = 3
      DataType = dtDouble
      DisplayWidth = 16
      Position = 4
    end
    object ppDBHistoricoppField6: TppField
      Alignment = taRightJustify
      FieldAlias = 'HIS_ATUAL'
      FieldName = 'HIS_ATUAL'
      FieldLength = 3
      DataType = dtDouble
      DisplayWidth = 16
      Position = 5
    end
    object ppDBHistoricoppField7: TppField
      FieldAlias = 'NOMEUSUARIO'
      FieldName = 'NOMEUSUARIO'
      FieldLength = 40
      DisplayWidth = 40
      Position = 6
    end
    object ppDBHistoricoppField8: TppField
      FieldAlias = 'HIS_NOMETIPO'
      FieldName = 'HIS_NOMETIPO'
      FieldLength = 15
      DisplayWidth = 15
      Position = 7
    end
    object ppDBHistoricoppField9: TppField
      Alignment = taRightJustify
      FieldAlias = 'HIS_MOVIMENTO'
      FieldName = 'HIS_MOVIMENTO'
      FieldLength = 3
      DataType = dtDouble
      DisplayWidth = 16
      Position = 8
    end
  end
  object ppReport1: TppReport
    AutoStop = False
    DataPipeline = ppDBHistorico
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    Template.FileName = 'C:\EMPRESA\SERVICO\DELPHI\Relatorios\rbRelHistEstProdutos.rtm'
    Units = utMillimeters
    DeviceType = 'Screen'
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = False
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zs100Percent
    TextSearchSettings.DefaultString = '<FindText>'
    TextSearchSettings.Enabled = True
    Left = 312
    Top = 40
    Version = '7.01'
    mmColumnWidth = 0
    DataPipelineName = 'ppDBHistorico'
    object ppHeaderBand1: TppHeaderBand
      mmBottomOffset = 0
      mmHeight = 34660
      mmPrintPosition = 0
      object ppDBImage1: TppDBImage
        UserName = 'DBImage1'
        MaintainAspectRatio = False
        Stretch = True
        DataField = 'CFG_IMAGEMLOGO'
        DataPipeline = Dm.ppDBParametros
        GraphicType = 'Bitmap'
        ParentDataPipeline = False
        DataPipelineName = 'ppDBParametros'
        mmHeight = 23283
        mmLeft = 3440
        mmTop = 2646
        mmWidth = 41540
        BandType = 0
      end
      object ppLabel4: TppLabel
        UserName = 'Label4'
        Caption = 'DATA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 4233
        mmLeft = 2646
        mmTop = 29369
        mmWidth = 9737
        BandType = 0
      end
      object ppLabel1: TppLabel
        UserName = 'Label1'
        Caption = 'TIPO MOVIMENTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 4233
        mmLeft = 19315
        mmTop = 29369
        mmWidth = 31581
        BandType = 0
      end
      object ppLine1: TppLine
        UserName = 'Line1'
        Weight = 0.750000000000000000
        mmHeight = 794
        mmLeft = 265
        mmTop = 33866
        mmWidth = 197380
        BandType = 0
      end
      object ppLine3: TppLine
        UserName = 'Line3'
        Style = lsDouble
        Weight = 0.750000000000000000
        mmHeight = 794
        mmLeft = 265
        mmTop = 28310
        mmWidth = 197380
        BandType = 0
      end
      object ppLabel8: TppLabel
        UserName = 'lbl_NMRELA2'
        Caption = 'RELAT'#211'RIO : HISTORICO DE ESTOQUE DE PRODUTOS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 4064
        mmLeft = 101071
        mmTop = 5027
        mmWidth = 94912
        BandType = 0
      end
      object txtPeriodo: TppLabel
        UserName = 'txtPeriodo'
        Caption = '...'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 4064
        mmLeft = 101071
        mmTop = 9525
        mmWidth = 3048
        BandType = 0
      end
      object txtProduto: TppLabel
        UserName = 'txtPeriodo1'
        Caption = '...'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 4064
        mmLeft = 101071
        mmTop = 14023
        mmWidth = 3048
        BandType = 0
      end
      object ppLabel2: TppLabel
        UserName = 'Label2'
        Caption = 'DOCUMENTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 4233
        mmLeft = 55298
        mmTop = 29369
        mmWidth = 23368
        BandType = 0
      end
      object ppLabel3: TppLabel
        UserName = 'Label3'
        Caption = 'EST.ANTERIOR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 4233
        mmLeft = 80698
        mmTop = 29369
        mmWidth = 26585
        BandType = 0
      end
      object ppLabel5: TppLabel
        UserName = 'Label5'
        Caption = 'MUNDAN'#199'A'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 4233
        mmLeft = 108479
        mmTop = 29369
        mmWidth = 20574
        BandType = 0
      end
      object ppLabel6: TppLabel
        UserName = 'Label6'
        Caption = 'EST. ATUAL'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 4233
        mmLeft = 130175
        mmTop = 29369
        mmWidth = 20997
        BandType = 0
      end
      object ppLabel7: TppLabel
        UserName = 'Label7'
        Caption = 'USUARIO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 4233
        mmLeft = 153194
        mmTop = 29369
        mmWidth = 16256
        BandType = 0
      end
    end
    object ppDetailBand1: TppDetailBand
      mmBottomOffset = 0
      mmHeight = 4763
      mmPrintPosition = 0
      object ppDBText1: TppDBText
        UserName = 'DBText1'
        DataField = 'HIS_DATA'
        DataPipeline = ppDBHistorico
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBHistorico'
        mmHeight = 4064
        mmLeft = 1323
        mmTop = 529
        mmWidth = 17198
        BandType = 4
      end
      object ppDBText3: TppDBText
        UserName = 'DBText3'
        DataField = 'HIS_NRDOCU'
        DataPipeline = ppDBHistorico
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBHistorico'
        mmHeight = 3969
        mmLeft = 55827
        mmTop = 529
        mmWidth = 22754
        BandType = 4
      end
      object ppDBText4: TppDBText
        UserName = 'DBText4'
        DataField = 'HIS_ANTERIOR'
        DataPipeline = ppDBHistorico
        DisplayFormat = '###,##0.#00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBHistorico'
        mmHeight = 3969
        mmLeft = 81756
        mmTop = 529
        mmWidth = 23813
        BandType = 4
      end
      object ppDBText5: TppDBText
        UserName = 'DBText5'
        DataField = 'HIS_MOVIMENTO'
        DataPipeline = ppDBHistorico
        DisplayFormat = '###,##0.#00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBHistorico'
        mmHeight = 3969
        mmLeft = 107950
        mmTop = 529
        mmWidth = 21431
        BandType = 4
      end
      object ppDBText6: TppDBText
        UserName = 'DBText6'
        DataField = 'HIS_ATUAL'
        DataPipeline = ppDBHistorico
        DisplayFormat = '###,##0.#00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBHistorico'
        mmHeight = 3969
        mmLeft = 130175
        mmTop = 529
        mmWidth = 19579
        BandType = 4
      end
      object ppDBText7: TppDBText
        UserName = 'DBText7'
        AutoSize = True
        DataField = 'NOMEUSUARIO'
        DataPipeline = ppDBHistorico
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBHistorico'
        mmHeight = 4064
        mmLeft = 152929
        mmTop = 529
        mmWidth = 26839
        BandType = 4
      end
      object ppDBText8: TppDBText
        UserName = 'DBText8'
        AutoSize = True
        DataField = 'HIS_NOMETIPO'
        DataPipeline = ppDBHistorico
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBHistorico'
        mmHeight = 3969
        mmLeft = 19315
        mmTop = 265
        mmWidth = 34925
        BandType = 4
      end
    end
    object ppFooterBand1: TppFooterBand
      mmBottomOffset = 0
      mmHeight = 13229
      mmPrintPosition = 0
      object ppLabel12: TppLabel
        UserName = 'Label12'
        Caption = 'Data/Hora:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 3969
        mmLeft = 143669
        mmTop = 2381
        mmWidth = 17198
        BandType = 8
      end
      object ppSystemVariable1: TppSystemVariable
        UserName = 'SystemVariable1'
        VarType = vtDateTime
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 3969
        mmLeft = 161661
        mmTop = 2381
        mmWidth = 32279
        BandType = 8
      end
      object ppLabel13: TppLabel
        UserName = 'Label13'
        Caption = 'P'#225'gina:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 3969
        mmLeft = 178330
        mmTop = 6879
        mmWidth = 11906
        BandType = 8
      end
      object ppSystemVariable2: TppSystemVariable
        UserName = 'SystemVariable2'
        VarType = vtPageCount
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 3969
        mmLeft = 191823
        mmTop = 6879
        mmWidth = 1852
        BandType = 8
      end
      object ppLine5: TppLine
        UserName = 'Line5'
        Style = lsDouble
        Weight = 0.750000000000000000
        mmHeight = 794
        mmLeft = 0
        mmTop = 529
        mmWidth = 197380
        BandType = 8
      end
    end
  end
end
