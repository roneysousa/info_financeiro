object frmManuContReceber: TfrmManuContReceber
  Left = 187
  Top = 164
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Manuten'#231#227'o de Contas a Receber'
  ClientHeight = 531
  ClientWidth = 880
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  ShowHint = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox1: TGroupBox
    Left = 0
    Top = 0
    Width = 880
    Height = 145
    Align = alTop
    Caption = 'Pesquisa'
    TabOrder = 0
    object Label6: TLabel
      Left = 11
      Top = 90
      Width = 45
      Height = 13
      Caption = '&Situa'#231#227'o:'
      FocusControl = edtNRCRED
      Transparent = True
    end
    object Label1: TLabel
      Left = 10
      Top = 54
      Width = 38
      Height = 13
      Caption = '&Per'#237'odo'
      Transparent = True
    end
    object Label2: TLabel
      Left = 325
      Top = 89
      Width = 66
      Height = 13
      Caption = '&N'#186' do Credito:'
      FocusControl = edtNRCRED
      Transparent = True
      Visible = False
    end
    object Label3: TLabel
      Left = 261
      Top = 54
      Width = 133
      Height = 13
      Caption = '&Loja\Comodat'#225'rio\Locat'#225'rio'
      FocusControl = edtCDCLIE
    end
    object spLocClientes: TSpeedButton
      Left = 382
      Top = 67
      Width = 23
      Height = 22
      Hint = 'Localizar Loja\Comodat'#225'rio\Locat'#225'rio'
      Flat = True
      Glyph.Data = {
        
        777777777770B777777777777777777777777777777777777777}
      OnClick = spLocClientesClick
    end
    object Label4: TLabel
      Left = 11
      Top = 14
      Width = 55
      Height = 13
      Caption = '&Condominio'
      FocusControl = cmbNMLOJA
      Transparent = True
    end
    object Label9: TLabel
      Left = 138
      Top = 90
      Width = 32
      Height = 13
      Caption = '&Grupo:'
      FocusControl = edtNRCRED
      Transparent = True
    end
    object cmbSituacao: TComboBox
      Left = 10
      Top = 105
      Width = 121
      Height = 22
      Style = csOwnerDrawFixed
      ItemHeight = 16
      TabOrder = 4
      OnChange = cmbSituacaoChange
      OnExit = cmbSituacaoExit
      OnKeyPress = cmbSituacaoKeyPress
      Items.Strings = (
        'ABERTAS'
        'PAGAS'
        'TODAS')
    end
    object edtDTINICIAL: TDateEdit
      Left = 10
      Top = 68
      Width = 121
      Height = 21
      NumGlyphs = 2
      TabOrder = 1
      OnChange = edtDTINICIALChange
      OnEnter = edtDTINICIALEnter
      OnExit = edtDTINICIALExit
      OnKeyPress = edtDTINICIALKeyPress
    end
    object edtDTFINAL: TDateEdit
      Left = 138
      Top = 68
      Width = 121
      Height = 21
      NumGlyphs = 2
      TabOrder = 2
      OnChange = edtDTFINALChange
      OnEnter = edtDTFINALEnter
      OnExit = edtDTFINALExit
      OnKeyPress = edtDTFINALKeyPress
    end
    object edtNRCRED: TEdit
      Left = 325
      Top = 108
      Width = 121
      Height = 21
      Hint = 'Digite o n'#250'mero do credito '
      MaxLength = 7
      TabOrder = 8
      Visible = False
      OnChange = edtNRCREDChange
      OnExit = edtNRCREDExit
      OnKeyPress = edtNRCREDKeyPress
    end
    object edtCDCLIE: TEdit
      Left = 261
      Top = 68
      Width = 121
      Height = 21
      MaxLength = 7
      TabOrder = 3
      OnChange = edtCDCLIEChange
      OnEnter = edtCDCLIEEnter
      OnExit = edtCDCLIEExit
      OnKeyPress = edtCDCLIEKeyPress
    end
    object btFiltar: TBitBtn
      Left = 681
      Top = 100
      Width = 75
      Height = 25
      Caption = 'Fil&tra'
      TabOrder = 7
      OnClick = btFiltarClick
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFA98FAFF00FFFF00FF}
    end
    object rgPeriodos: TRadioGroup
      Left = 388
      Top = 93
      Width = 282
      Height = 36
      Caption = '[ Per'#237'odos ]'
      Columns = 3
      ItemIndex = 0
      Items.Strings = (
        '&Emiss'#227'o'
        '&Vencimento'
        'Pa&gamento')
      TabOrder = 6
    end
    object cmbNMLOJA: TComboBox
      Left = 10
      Top = 30
      Width = 249
      Height = 22
      Style = csOwnerDrawFixed
      ItemHeight = 16
      TabOrder = 0
      OnExit = cmbNMLOJAExit
      OnKeyPress = cmbNMLOJAKeyPress
    end
    object lbl_NMCLIE: TEdit
      Left = 409
      Top = 68
      Width = 384
      Height = 21
      TabStop = False
      Color = clSilver
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ReadOnly = True
      TabOrder = 9
    end
    object cmbGrupos: TComboBox
      Left = 138
      Top = 105
      Width = 243
      Height = 22
      Style = csOwnerDrawFixed
      ItemHeight = 16
      TabOrder = 5
    end
    object rbOrdem: TRadioGroup
      Left = 409
      Top = 18
      Width = 384
      Height = 37
      Caption = '[ Ordenar por ]'
      Columns = 4
      ItemIndex = 0
      Items.Strings = (
        'Nosso N'#250'mero'
        'Dt.Vencimento'
        'Dt.Pagamento')
      TabOrder = 10
      OnClick = rbOrdemClick
    end
  end
  object pnlGrid: TPanel
    Left = 0
    Top = 145
    Width = 880
    Height = 285
    Align = alClient
    TabOrder = 1
    object grdConsultar: TDBGrid
      Left = 1
      Top = 1
      Width = 878
      Height = 283
      Align = alClient
      Ctl3D = False
      DataSource = dsContas
      Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnCellClick = grdConsultarCellClick
      OnColEnter = grdConsultarColEnter
      OnDrawColumnCell = grdConsultarDrawColumnCell
      OnDblClick = grdConsultarDblClick
      Columns = <
        item
          Expanded = False
          FieldName = 'REC_FLBOLETO'
          Title.Alignment = taCenter
          Title.Caption = 'IMP.'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REC_CREDITO'
          Title.Alignment = taCenter
          Title.Caption = 'CREDITO'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'REC_PARCELA'
          Title.Alignment = taCenter
          Title.Caption = 'PC'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 42
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'REC_DATAEMISSAO'
          Title.Alignment = taCenter
          Title.Caption = 'DATA EMISS'#195'O'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'REC_DATAVENCIMENTO'
          Title.Alignment = taCenter
          Title.Caption = 'VENCIMENTO'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 86
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'REC_DATAPAGAMENTO'
          Title.Alignment = taCenter
          Title.Caption = 'PAGAMENTO'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REC_NMCLIENTE'
          Title.Caption = 'LOJA\COMODAT'#193'RIO\LOCAT'#193'RIO'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 235
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REC_VALORPARCELA'
          Title.Alignment = taRightJustify
          Title.Caption = 'VALOR'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 77
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REC_PAGO'
          Title.Alignment = taRightJustify
          Title.Caption = 'VL PAGO'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 83
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REC_VALORJUROS'
          Title.Alignment = taRightJustify
          Title.Caption = 'VL.JUROS'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'REC_VALORDESCONTO'
          Title.Alignment = taRightJustify
          Title.Caption = 'VL.DESC.'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 65
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'REC_NMSITUACAO'
          Title.Caption = 'SITUA'#199#195'O'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 68
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'REC_NOSSO_NUMERO'
          Title.Alignment = taCenter
          Title.Caption = 'NOSSO N'#218'MERO'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 119
          Visible = True
        end
        item
          Alignment = taRightJustify
          Expanded = False
          FieldName = 'REC_SEUNUMERO'
          Title.Alignment = taRightJustify
          Title.Caption = 'N'#186'. DOCUM.'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 80
          Visible = True
        end
        item
          Alignment = taRightJustify
          Expanded = False
          FieldName = 'REC_NRFATURA'
          Title.Caption = 'N'#186' FATURA'
          Title.Font.Charset = ANSI_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'Verdana'
          Title.Font.Style = [fsBold]
          Width = 77
          Visible = True
        end>
    end
  end
  object pnlEdicao: TPanel
    Left = 0
    Top = 430
    Width = 880
    Height = 60
    Align = alBottom
    TabOrder = 2
    object Label5: TLabel
      Left = 11
      Top = 5
      Width = 171
      Height = 13
      Caption = 'TOTAL DE ABERTAS     ===>'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Label7: TLabel
      Left = 11
      Top = 21
      Width = 172
      Height = 13
      Caption = 'TOTAL DE RECEBIDAS  ===>'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object lbl_TOTABERTAS: TLabel
      Left = 265
      Top = 3
      Width = 29
      Height = 16
      Alignment = taRightJustify
      Caption = '0,00'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object lbl_RECEBIDAS: TLabel
      Left = 265
      Top = 20
      Width = 29
      Height = 16
      Alignment = taRightJustify
      Caption = '0,00'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Label8: TLabel
      Left = 11
      Top = 37
      Width = 182
      Height = 13
      Caption = 'TOTAL DE REGISTRO(S)  ===>'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object lblRegistro: TLabel
      Left = 285
      Top = 36
      Width = 9
      Height = 16
      Alignment = taRightJustify
      Caption = '0'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
    end
    object Memo1: TMemo
      Left = 344
      Top = 8
      Width = 185
      Height = 89
      Lines.Strings = (
        'DESCONTO DE XXX ATE '
        '99/99/9999'
        'JUROS DE MORA DE 2.0% '
        'MENSAL ()'
        'MULTA DE 2.0% A PARTIR DE '
        '99/99/9999')
      TabOrder = 0
      Visible = False
    end
    object DBNavigator1: TDBNavigator
      Left = 664
      Top = 5
      Width = 180
      Height = 25
      DataSource = dsContas
      VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast]
      TabOrder = 1
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 490
    Width = 880
    Height = 41
    Align = alBottom
    Color = clNavy
    TabOrder = 3
    object BtSair: TBitBtn
      Left = 772
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Fechar janela'
      Caption = '&Sair'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      OnClick = BtSairClick
      Glyph.Data = {
        
        1B1C010303030303031A1A1A1A1A1A1A0303011A1A1A1A1A1A1A}
    end
    object BtEditar: TBitBtn
      Left = 19
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Editar Parcela'
      Caption = '&Editar'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = BtEditarClick
      Glyph.Data = {
        
        3F3F3F3F39111616191919193334343434333334270D19191919}
    end
    object BtExcluir: TBitBtn
      Left = 104
      Top = 8
      Width = 75
      Height = 25
      Caption = 'E&xcluir'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BtExcluirClick
      Glyph.Data = {
        
        0404040404040404040404040404040404040404040404040404}
    end
    object btImprimir: TBitBtn
      Left = 195
      Top = 8
      Width = 75
      Height = 25
      Hint = '2'#170' Via de Recebimento'
      Caption = '2'#170' Via'
      Enabled = False
      TabOrder = 3
      Visible = False
      OnClick = btImprimirClick
      Glyph.Data = {
        
        0000000000000000DBDBDBDBDBDBDBDBDBDBDBDBDBDBDBDBDBDB}
    end
    object btnBoleto: TBitBtn
      Left = 464
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Imprime boletos selecionado'
      Caption = '&Boleto'
      Enabled = False
      TabOrder = 4
      Visible = False
      OnClick = btnBoletoClick
      Glyph.Data = {
        
        9292929292929292929292929292929292929292929292929292}
    end
    object btnBolEnviados: TBitBtn
      Left = 552
      Top = 8
      Width = 75
      Height = 25
      Hint = 'Rela'#231#227'o de Boletos Enviados'
      Caption = 'Bol.Enviados'
      Enabled = False
      TabOrder = 5
      Visible = False
      OnClick = btnBolEnviadosClick
    end
    object bntGerarBoletos: TBitBtn
      Left = 320
      Top = 8
      Width = 105
      Height = 25
      Caption = 'Gerar Boletos'
      TabOrder = 6
      OnClick = bntGerarBoletosClick
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFF92FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
  end
  object dstContasReceber: TSQLDataSet
    CommandText = 
      'Select REC_SEQUENCIA, REC_LOJA, REC_CREDITO, REC_PARCELA, REC_QT' +
      'PARCELAS, REC_CLIENTE, REC_NRVENDA, REC_DATAEMISSAO, REC_DATAVEN' +
      'CIMENTO, REC_VALORPARCELA, REC_PAGO, REC_SITUACAO, REC_DATAPAGAM' +
      'ENTO, REC_FLBOLETO, REC_NOSSO_NUMERO, REC_VLMULTA, REC_SEUNUMERO' +
      ', REC_VALORJUROS, REC_VALORDESCONTO, REC_NRFATURA, REC_DESCRICAO' +
      ', CL.cli_cdgrupo from CONTASRECEBER Inner Join CLIENTES CL on CL' +
      '.cli_codigo = CONTASRECEBER.rec_cliente where (REC_DATAVENCIMENT' +
      'O >= :pDTINICIO) AND (REC_DATAVENCIMENTO <= :pDTFINAL)'
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftDateTime
        Name = 'pDTINICIO'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = 'pDTFINAL'
        ParamType = ptInput
      end>
    SQLConnection = Dm.SqlAdmin
    Left = 344
    Top = 216
  end
  object dspContasReceber: TDataSetProvider
    DataSet = dstContasReceber
    Options = [poAllowCommandText]
    OnGetTableName = dspContasReceberGetTableName
    Left = 416
    Top = 216
  end
  object cdsContasReceber: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDateTime
        Name = 'pDTINICIO'
        ParamType = ptInput
      end
      item
        DataType = ftDateTime
        Name = 'pDTFINAL'
        ParamType = ptInput
      end>
    ProviderName = 'dspContasReceber'
    OnCalcFields = cdsContasReceberCalcFields
    Left = 480
    Top = 216
    object cdsContasReceberREC_SEQUENCIA: TIntegerField
      FieldName = 'REC_SEQUENCIA'
      Required = True
    end
    object cdsContasReceberREC_LOJA: TIntegerField
      FieldName = 'REC_LOJA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      DisplayFormat = '000'
    end
    object cdsContasReceberREC_CREDITO: TIntegerField
      Alignment = taCenter
      FieldName = 'REC_CREDITO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      DisplayFormat = '0000000'
    end
    object cdsContasReceberREC_PARCELA: TIntegerField
      FieldName = 'REC_PARCELA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      DisplayFormat = '00'
    end
    object cdsContasReceberREC_QTPARCELAS: TIntegerField
      FieldName = 'REC_QTPARCELAS'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = '00'
    end
    object cdsContasReceberREC_CLIENTE: TIntegerField
      FieldName = 'REC_CLIENTE'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = '0000000'
    end
    object cdsContasReceberREC_NRVENDA: TIntegerField
      FieldName = 'REC_NRVENDA'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = '0000000'
    end
    object cdsContasReceberREC_DATAEMISSAO: TDateField
      FieldName = 'REC_DATAEMISSAO'
      ProviderFlags = [pfInUpdate]
      EditMask = '99/99/9999;1;_'
    end
    object cdsContasReceberREC_DATAVENCIMENTO: TDateField
      FieldName = 'REC_DATAVENCIMENTO'
      ProviderFlags = [pfInUpdate]
      EditMask = '99/99/9999;1;_'
    end
    object cdsContasReceberREC_VALORPARCELA: TFMTBCDField
      FieldName = 'REC_VALORPARCELA'
      ProviderFlags = [pfInUpdate]
      DisplayFormat = '###,###,##0.#0'
      EditFormat = '###,###,##0.#0'
      Precision = 15
      Size = 2
    end
    object cdsContasReceberREC_SITUACAO: TStringField
      FieldName = 'REC_SITUACAO'
      ProviderFlags = [pfInUpdate]
      FixedChar = True
      Size = 1
    end
    object cdsContasReceberREC_DATAPAGAMENTO: TDateField
      FieldName = 'REC_DATAPAGAMENTO'
      ProviderFlags = [pfInUpdate]
      EditMask = '99/99/9999;1;'
    end
    object cdsContasReceberREC_NMSITUACAO: TStringField
      FieldKind = fkCalculated
      FieldName = 'REC_NMSITUACAO'
      Size = 15
      Calculated = True
    end
    object cdsContasReceberREC_ATRASO: TFloatField
      FieldKind = fkCalculated
      FieldName = 'REC_ATRASO'
      Calculated = True
    end
    object cdsContasReceberREC_NMCLIENTE: TStringField
      FieldKind = fkLookup
      FieldName = 'REC_NMCLIENTE'
      LookupDataSet = DM2.cdsClientes
      LookupKeyFields = 'CLI_CODIGO'
      LookupResultField = 'CLI_FANTASIA'
      KeyFields = 'REC_CLIENTE'
      Size = 40
      Lookup = True
    end
    object cdsContasReceberREC_FLBOLETO: TStringField
      FieldName = 'REC_FLBOLETO'
      FixedChar = True
      Size = 1
    end
    object cdsContasReceberREC_FLBOLE: TBooleanField
      FieldKind = fkCalculated
      FieldName = 'REC_FLBOLE'
      Calculated = True
    end
    object cdsContasReceberREC_PAGO: TFMTBCDField
      FieldName = 'REC_PAGO'
      DisplayFormat = '###,##0.#0'
      EditFormat = '###,##0.#0'
      Precision = 18
      Size = 2
    end
    object cdsContasReceberREC_VLMULTA: TFMTBCDField
      FieldName = 'REC_VLMULTA'
      DisplayFormat = '###,###,##0.#0'
      EditFormat = '###,###,##0.#0'
      Precision = 18
      Size = 2
    end
    object cdsContasReceberREC_NOSSO_NUMERO: TStringField
      FieldName = 'REC_NOSSO_NUMERO'
    end
    object cdsContasReceberREC_SEUNUMERO: TStringField
      FieldName = 'REC_SEUNUMERO'
      Size = 15
    end
    object cdsContasReceberREC_VALORJUROS: TFMTBCDField
      FieldName = 'REC_VALORJUROS'
      DisplayFormat = '###,###,##0.#0'
      EditFormat = '###,###,##0.#0'
      Precision = 18
      Size = 2
    end
    object cdsContasReceberREC_VALORDESCONTO: TFMTBCDField
      FieldName = 'REC_VALORDESCONTO'
      DisplayFormat = '###,###,##0.#0'
      EditFormat = '###,###,##0.#0'
      Precision = 18
      Size = 2
    end
    object cdsContasReceberREC_NRFATURA: TStringField
      FieldName = 'REC_NRFATURA'
      FixedChar = True
      Size = 10
    end
    object cdsContasReceberREC_DESCRICAO: TMemoField
      FieldName = 'REC_DESCRICAO'
      BlobType = ftMemo
    end
    object cdsContasReceberCLI_CDGRUPO: TIntegerField
      FieldName = 'CLI_CDGRUPO'
    end
  end
  object dsContas: TDataSource
    DataSet = cdsContasReceber
    OnDataChange = dsContasDataChange
    Left = 536
    Top = 217
  end
  object ImageList1: TImageList
    Left = 576
    Top = 64
    Bitmap = {
      
      000000000000}
  end
  object cdsDados: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    Left = 456
    Top = 265
    Data = {
      
      4E4304000600000000000000}
    object cdsDadosCODIGO: TStringField
      FieldName = 'CODIGO'
      Size = 7
    end
    object cdsDadosNOME: TStringField
      FieldName = 'NOME'
      Size = 40
    end
    object cdsDadosDOCUMENTO: TStringField
      FieldName = 'DOCUMENTO'
      Size = 10
    end
    object cdsDadosDT_ORIGINAL: TDateField
      FieldName = 'DT_ORIGINAL'
    end
    object cdsDadosATRASO: TFloatField
      FieldName = 'ATRASO'
    end
    object cdsDadosVL_DOCUMENTO: TCurrencyField
      FieldName = 'VL_DOCUMENTO'
    end
    object cdsDadosDT_VENC: TDateField
      FieldName = 'DT_VENC'
    end
  end
  object ppReport1: TppReport
    AutoStop = False
    DataPipeline = ppDBBolEnviados
    PrinterSetup.BinName = 'Default'
    PrinterSetup.DocumentName = 'Report'
    PrinterSetup.PaperName = 'A4'
    PrinterSetup.PrinterName = 'Default'
    PrinterSetup.mmMarginBottom = 6350
    PrinterSetup.mmMarginLeft = 6350
    PrinterSetup.mmMarginRight = 6350
    PrinterSetup.mmMarginTop = 6350
    PrinterSetup.mmPaperHeight = 297000
    PrinterSetup.mmPaperWidth = 210000
    PrinterSetup.PaperSize = 9
    Template.FileName = 'C:\Empresa\SERVICO\DELPHI\Relatorios\rbRelacaoBolEnviados.rtm'
    Units = utMillimeters
    DeviceType = 'Screen'
    OutlineSettings.CreateNode = True
    OutlineSettings.CreatePageNodes = True
    OutlineSettings.Enabled = True
    OutlineSettings.Visible = False
    PreviewFormSettings.WindowState = wsMaximized
    PreviewFormSettings.ZoomSetting = zs100Percent
    TextSearchSettings.DefaultString = '<FindText>'
    TextSearchSettings.Enabled = True
    Left = 408
    Top = 261
    Version = '7.01'
    mmColumnWidth = 0
    DataPipelineName = 'ppDBBolEnviados'
    object ppHeaderBand1: TppHeaderBand
      mmBottomOffset = 0
      mmHeight = 27252
      mmPrintPosition = 0
      object ppLabel1: TppLabel
        UserName = 'Label1'
        Caption = 'C'#211'DIGO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3895
        mmLeft = 529
        mmTop = 22490
        mmWidth = 12171
        BandType = 0
      end
      object ppLabel2: TppLabel
        UserName = 'Label2'
        Caption = 'NOME'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 3895
        mmLeft = 14552
        mmTop = 22490
        mmWidth = 9737
        BandType = 0
      end
      object ppLabel3: TppLabel
        UserName = 'Label3'
        Caption = 'DOCUMENTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 3704
        mmLeft = 92340
        mmTop = 22490
        mmWidth = 20955
        BandType = 0
      end
      object ppLabel4: TppLabel
        UserName = 'Label4'
        Caption = 'DT.VENCIM.'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 3704
        mmLeft = 139965
        mmTop = 22490
        mmWidth = 18203
        BandType = 0
      end
      object ppLine1: TppLine
        UserName = 'Line1'
        Pen.Style = psDot
        Weight = 0.750000000000000000
        mmHeight = 794
        mmLeft = 0
        mmTop = 21431
        mmWidth = 197380
        BandType = 0
      end
      object ppLine2: TppLine
        UserName = 'Line2'
        Pen.Style = psDot
        Weight = 0.750000000000000000
        mmHeight = 794
        mmLeft = 265
        mmTop = 27252
        mmWidth = 197380
        BandType = 0
      end
      object ppLabel6: TppLabel
        UserName = 'Label6'
        Caption = 'RELA'#199#195'O DE BOLETOS ENVIADOS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 12
        Font.Style = [fsBold]
        Transparent = True
        mmHeight = 4939
        mmLeft = 57415
        mmTop = 1588
        mmWidth = 72178
        BandType = 0
      end
      object ppLabel12: TppLabel
        UserName = 'Label12'
        Caption = 'VALOR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        mmHeight = 3969
        mmLeft = 184680
        mmTop = 22490
        mmWidth = 11113
        BandType = 0
      end
      object ppLabel5: TppLabel
        UserName = 'Label5'
        Caption = 'DT.EMISS'#195'O'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = [fsBold]
        TextAlignment = taCentered
        Transparent = True
        mmHeight = 3704
        mmLeft = 116946
        mmTop = 22490
        mmWidth = 19685
        BandType = 0
      end
      object ppDBImage1: TppDBImage
        UserName = 'DBImage1'
        MaintainAspectRatio = False
        Stretch = True
        DataField = 'CFG_IMAGEMLOGO'
        DataPipeline = Dm.ppDBParametros
        GraphicType = 'Bitmap'
        ParentDataPipeline = False
        DataPipelineName = 'ppDBParametros'
        mmHeight = 19315
        mmLeft = 1323
        mmTop = 1058
        mmWidth = 28575
        BandType = 0
      end
      object lblPeriodo: TppLabel
        UserName = 'lblPeriodo'
        Caption = 'Per'#237'odo : 99/99/9999 '#192' 99/99/9999'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 4057
        mmLeft = 141288
        mmTop = 16140
        mmWidth = 54610
        BandType = 0
      end
    end
    object ppDetailBand1: TppDetailBand
      mmBottomOffset = 0
      mmHeight = 4233
      mmPrintPosition = 0
      object ppDBText1: TppDBText
        UserName = 'DBText1'
        DataField = 'CODIGO'
        DataPipeline = ppDBBolEnviados
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBBolEnviados'
        mmHeight = 3725
        mmLeft = 265
        mmTop = 265
        mmWidth = 13494
        BandType = 4
      end
      object ppDBText2: TppDBText
        UserName = 'DBText2'
        DataField = 'NOME'
        DataPipeline = ppDBBolEnviados
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBBolEnviados'
        mmHeight = 3704
        mmLeft = 14552
        mmTop = 265
        mmWidth = 67204
        BandType = 4
      end
      object ppDBText3: TppDBText
        UserName = 'DBText3'
        DataField = 'DOCUMENTO'
        DataPipeline = ppDBBolEnviados
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBBolEnviados'
        mmHeight = 3725
        mmLeft = 92340
        mmTop = 265
        mmWidth = 20373
        BandType = 4
      end
      object ppDBText4: TppDBText
        UserName = 'DBText4'
        DataField = 'DT_VENC'
        DataPipeline = ppDBBolEnviados
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBBolEnviados'
        mmHeight = 3725
        mmLeft = 139965
        mmTop = 265
        mmWidth = 17865
        BandType = 4
      end
      object ppDBText8: TppDBText
        UserName = 'DBText8'
        DataField = 'VL_DOCUMENTO'
        DataPipeline = ppDBBolEnviados
        DisplayFormat = '###,##0.#0'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBBolEnviados'
        mmHeight = 3704
        mmLeft = 172509
        mmTop = 265
        mmWidth = 23813
        BandType = 4
      end
      object ppDBText5: TppDBText
        UserName = 'DBText5'
        DataField = 'DT_ORIGINAL'
        DataPipeline = ppDBBolEnviados
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 9
        Font.Style = []
        Transparent = True
        DataPipelineName = 'ppDBBolEnviados'
        mmHeight = 3704
        mmLeft = 117211
        mmTop = 265
        mmWidth = 17992
        BandType = 4
      end
    end
    object ppFooterBand1: TppFooterBand
      mmBottomOffset = 0
      mmHeight = 13229
      mmPrintPosition = 0
      object ppLine4: TppLine
        UserName = 'Line4'
        Pen.Style = psInsideFrame
        Style = lsDouble
        Weight = 0.750000000000000000
        mmHeight = 794
        mmLeft = 0
        mmTop = 794
        mmWidth = 197380
        BandType = 8
      end
      object ppLabel7: TppLabel
        UserName = 'Label7'
        Caption = 'Data/Hora:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 3969
        mmLeft = 144727
        mmTop = 2646
        mmWidth = 17198
        BandType = 8
      end
      object ppLabel8: TppLabel
        UserName = 'Label8'
        Caption = 'P'#225'gina:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 4064
        mmLeft = 149754
        mmTop = 7408
        mmWidth = 12023
        BandType = 8
      end
      object ppSystemVariable1: TppSystemVariable
        UserName = 'SystemVariable1'
        VarType = vtDateTime
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 4064
        mmLeft = 162719
        mmTop = 2646
        mmWidth = 32343
        BandType = 8
      end
      object ppSystemVariable2: TppSystemVariable
        UserName = 'SystemVariable2'
        VarType = vtPageNo
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 4064
        mmLeft = 162984
        mmTop = 7408
        mmWidth = 1947
        BandType = 8
      end
    end
    object ppSummaryBand1: TppSummaryBand
      mmBottomOffset = 0
      mmHeight = 13229
      mmPrintPosition = 0
      object ppLine3: TppLine
        UserName = 'Line3'
        Pen.Style = psDot
        Weight = 0.750000000000000000
        mmHeight = 794
        mmLeft = 0
        mmTop = 265
        mmWidth = 197380
        BandType = 7
      end
      object ppLabel9: TppLabel
        UserName = 'Label9'
        Caption = 'REGISTRO(S):'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = []
        Transparent = True
        mmHeight = 4064
        mmLeft = 794
        mmTop = 1323
        mmWidth = 24384
        BandType = 7
      end
      object ppDBCalc2: TppDBCalc
        UserName = 'DBCalc2'
        DataPipeline = ppDBBolEnviados
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        Transparent = True
        DBCalcType = dcCount
        DataPipelineName = 'ppDBBolEnviados'
        mmHeight = 4233
        mmLeft = 26458
        mmTop = 1323
        mmWidth = 17198
        BandType = 7
      end
      object ppDBCalc3: TppDBCalc
        UserName = 'DBCalc3'
        DataField = 'VL_DOCUMENTO'
        DataPipeline = ppDBBolEnviados
        DisplayFormat = '###,###,##0.00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Name = 'Arial'
        Font.Size = 10
        Font.Style = [fsBold]
        TextAlignment = taRightJustified
        Transparent = True
        DataPipelineName = 'ppDBBolEnviados'
        mmHeight = 4233
        mmLeft = 172509
        mmTop = 1323
        mmWidth = 23813
        BandType = 7
      end
    end
  end
  object ppDBBolEnviados: TppDBPipeline
    DataSource = dsBolReceber
    UserName = 'DBBolEnviados'
    Left = 552
    Top = 265
    object ppDBPipeline1ppField1: TppField
      FieldAlias = 'CODIGO'
      FieldName = 'CODIGO'
      FieldLength = 7
      DisplayWidth = 7
      Position = 0
    end
    object ppDBPipeline1ppField2: TppField
      FieldAlias = 'NOME'
      FieldName = 'NOME'
      FieldLength = 40
      DisplayWidth = 40
      Position = 1
    end
    object ppDBPipeline1ppField3: TppField
      FieldAlias = 'DOCUMENTO'
      FieldName = 'DOCUMENTO'
      FieldLength = 10
      DisplayWidth = 10
      Position = 2
    end
    object ppDBPipeline1ppField4: TppField
      FieldAlias = 'DT_ORIGINAL'
      FieldName = 'DT_ORIGINAL'
      FieldLength = 0
      DataType = dtDate
      DisplayWidth = 10
      Position = 3
    end
    object ppDBPipeline1ppField5: TppField
      Alignment = taRightJustify
      FieldAlias = 'ATRASO'
      FieldName = 'ATRASO'
      FieldLength = 0
      DataType = dtDouble
      DisplayWidth = 10
      Position = 4
    end
    object ppDBPipeline1ppField6: TppField
      FieldAlias = 'VL_DOCUMENTO'
      FieldName = 'VL_DOCUMENTO'
      FieldLength = 0
      DataType = dtCurrency
      DisplayWidth = 10
      Position = 5
    end
    object ppDBPipeline1ppField7: TppField
      FieldAlias = 'DT_VENC'
      FieldName = 'DT_VENC'
      FieldLength = 0
      DataType = dtDate
      DisplayWidth = 10
      Position = 6
    end
  end
  object dsBolReceber: TDataSource
    DataSet = cdsDados
    Left = 504
    Top = 265
  end
  object RvSystem1: TRvSystem
    TitleSetup = 'Output Options'
    TitleStatus = 'Report Status'
    TitlePreview = 'Report Preview'
    SystemFiler.StatusFormat = 'Generating page %p'
    SystemPreview.ZoomFactor = 100.000000000000000000
    SystemPrinter.ScaleX = 100.000000000000000000
    SystemPrinter.ScaleY = 100.000000000000000000
    SystemPrinter.StatusFormat = 'Printing page %p'
    SystemPrinter.Title = 'ReportPrinter Report'
    SystemPrinter.UnitsFactor = 1.000000000000000000
    Left = 616
    Top = 64
  end
end
